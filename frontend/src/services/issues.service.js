  ...
  const [dataResult, countResult] = await Promise.all([
    pool.query(
      `SELECT ${ISSUE_SELECT},
              COALESCE(
                ARRAY(
              SELECT json_build_object('id', m.id, 'url', m.media_url, 'type', m."mediaType")
...
async function getIssueById(issueId) {
  const result = await pool.query(
    `SELECT ${ISSUE_SELECT},
            ...
            COALESCE(
              ARRAY(
              SELECT json_build_object('id', m.id, 'url', m.media_url, 'type', m."mediaType", 'is_resolution', m.is_resolution)
              FROM issue_media m WHERE m.issue_id = i.id
              ),
              ARRAY[]::json[]
            ) AS media
 ...
 */
async function insertIssueMedia(client, { issueId, url, publicId, type, isResolution = false }) {
  const id = require('crypto').randomUUID();
  await client.query(
    `INSERT INTO issue_media (id, issue_id, media_url, public_id, "mediaType", is_resolution)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [id, issueId, url, publicId || null, type, isResolution]
  );
}
...
async function insertStatusHistory(client, { issueId, oldStatus, newStatus, updatedBy, comment, isPublic = true }) {
  ...
  await client.query(
    `INSERT INTO issue_status_history
       (id, issue_id, old_status, new_status, updated_by, comment, is_public)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
    [id, issueId, oldStatus || null, newStatus, updatedBy, comment || null, isPublic]
  );
}
...
async function notifyFollowers(client, { issueId, issueTitle, newStatus, actorId }) {
  const sql = `
    ...
    ${actorId ? 'WHERE user_id != $2' : ''}
  `;
  const params = actorId ? [issueId, actorId] : [issueId];
